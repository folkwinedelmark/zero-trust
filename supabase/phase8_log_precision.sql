-- =============================================================================
-- ZERO TRUST — Log spaziali precisi + Kick con handle noto (intel Trace)
-- Esegui dopo phase7_intel_logs.sql
-- =============================================================================
  ...
  values (
    ...
    case
      when v_outcome = 'failure' then
        format(
          'Trace fallito (segnale perso) — Server: %s [Slot %s]',
          v_node_name,
          coalesce(v_target_slot_label, v_slot.slot_id::text)
        )
      else
        format(
          'Trace completato su %s — Server: %s [Slot %s]',
          v_revealed,
          v_node_name,
          coalesce(v_target_slot_label, v_slot.slot_id::text)
        )
...
-- p_known_handle: intel client da Trace precedente (evita Unknown su Kick)
...
begin
  ...
  -- Nome da mostrare: DB > intel Trace client > Unknown
  v_display_name := coalesce(
    nullif(trim(v_target_name), ''),
    nullif(trim(p_known_handle), ''),
    'Unknown'
  );
  ...
  values (
    ...
    case
      when v_outcome = 'success' then
        format(
          'Kick eseguito su %s — account BLOCKED — Server: %s [Slot %s]',
          v_display_name,
          v_node_name,
          coalesce(v_target_slot_label, v_slot.slot_id::text)
        )
        ...
        format(
          'Kick vanificato su %s (fuga/Abort) — Server: %s [Slot %s]',
          v_display_name,
          v_node_name,
          coalesce(v_target_slot_label, v_slot.slot_id::text)
        )
